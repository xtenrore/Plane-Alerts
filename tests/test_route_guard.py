...
def prediction(*, current=20.0, cpa=2.0, cpa_time=120.0, entry=90.0, path=(), stale=False):
    return SimpleNamespace(
...
@pytest.mark.asyncio
async def test_missing_coverage_is_inconclusive_not_a_route_veto():
    ac = aircraft("THY405")
    key = route_mod.normalize_flight_key(ac.callsign)
    destination_resolver._cache[key] = CacheEntry(
        None, "unavailable", time.monotonic() + 30
    )
    result = await gate(ac, prediction(stale=True))
    assert not result.suppress_alert
    assert result.destination_code == ""
...
@pytest.mark.asyncio
async def test_late_destination_resolution_can_veto_previously_qualified_alert():
    ac = aircraft(
        "MNB313", 40.90, 28.73,
        heading=42, altitude=1800, vertical_rate=-5,
    )
    ...
    assert not before.suppress_alert
    ...
    assert after.suppress_alert
    assert after.destination_code == "ISL"
    assert "before" in after.reason.lower()
...
def test_production_runtime_installs_only_canonical_route_guard():
