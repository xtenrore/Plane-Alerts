...
def _assert_prediction_equivalent(left, right) -> None:
    scalar_fields = (
        ...
        "stale",
        "three_d_available",
        "observer_altitude_known",
        "altitude_confidence",
        "altitude_relevance_applied",
        "altitude_relevance_reason",
        "reason",
    )
    ...
    numeric_fields = (
        ...
        "time_to_3d_cpa_s",
        "horizontal_at_3d_cpa_km",
        "vertical_at_3d_cpa_m",
...
def test_shared_motion_is_output_equivalent_for_multiple_observers():
    ...
    observers = [
        (41.25, 28.95, 15.0, 80.0),
        (41.32, 29.02, 25.0, None),
        (41.10, 28.80, 10.0, 150.0),
    ]
    for lat, lon, radius, elevation in observers:
        expected = base.predict_trajectory(
            samples,
            lat,
            lon,
            radius,
            now=now,
            user_altitude_m=elevation,
        )
        actual = scaled.predict_trajectory(
            samples,
            lat,
            lon,
            radius,
...
def test_spatial_index_keeps_user_state_isolated():
    ...
    aircraft = [
        _aircraft("abc001", 41.05, 29.05),
        _aircraft("abc002", 41.05, 32.05),
    ]
...
def test_spatial_index_matches_bruteforce_candidate_truth():
    users = [
        _user(uid, 40.8 + (uid % 10) * 0.05, 28.6 + (uid // 10) * 0.05, radius=20.0)
        for uid in range(1, 81)
    ]
    aircraft = [
        _aircraft(f"{index:06x}", 40.6 + (index % 20) * 0.07, 28.3 + (index // 20) * 0.08)
    ...
    for user in users:
        ...
        brute = {
            ac.icao24
            for ac in aircraft
            if scale_v53.haversine(loc["latitude"], loc["longitude"], ac.latitude, ac.longitude) <= threshold
        }
