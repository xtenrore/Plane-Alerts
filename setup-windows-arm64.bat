...
set "TARGET_LABEL=Windows ARM64"
...
if /I "%~1"=="--self-test" (
  echo INSTALLER_SELF_TEST=ok platform=windows-arm64 AGY=excluded
  exit /b 0
)
...
if /I not "%ARCH%"=="%TARGET_ARCH%" (
  ...
  echo Download setup-windows.bat for Intel/AMD 64-bit Windows.
  exit /b 2
)
if /I "%~1"=="--check-platform" (
  echo PLATFORM_CHECK=ok platform=windows-arm64 arch=%ARCH%
  exit /b 0
)
...
where py >nul 2>&1
if errorlevel 1 (
  echo Installing native Python 3.11 from the official winget package...
...
if errorlevel 1 (popd & echo ERROR: Repository identity check failed. & exit /b 12)
