...
if [[ "${1:-}" == "--self-test" ]]; then
  echo "INSTALLER_SELF_TEST=ok platform=linux-arm64 AGY=excluded"
  exit 0
fi

ARCH="$(uname -m)"
if [[ "$ARCH" != "$TARGET_ARCH" && "$ARCH" != "arm64" ]]; then
  echo "ERROR: This installer is for $TARGET_LABEL. Detected architecture: $ARCH" >&2
  echo "Use setup-linux.sh on Intel/AMD x64 systems." >&2
  exit 2
fi
if [[ "${1:-}" == "--check-platform" ]]; then
  echo "PLATFORM_CHECK=ok platform=linux-arm64 arch=$ARCH"
  exit 0
fi
...
if [[ ! -r /etc/os-release ]]; then
  echo "ERROR: This installer supports tested Debian/Ubuntu-family ARM64 systems." >&2
  exit 6
fi
